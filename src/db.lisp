...
;;;
;; There are two reference tables: reference and reference2.
;; - The first is for the reference field in an opinion.
;; - Reference2 is for additional references. Any links that are placed in an opinion
;; comment count as additional.
;; - Also, target articles can contain references: any links found in the body of the
;; article should be counted as references. Determination of what links should be extracted
;; from the target is not determined here.
;; - We might consider extracting references from articles even if they aren't rooturls.
;; - To get all of the relevant references, both tables will need to be checked.
;;;

(defun get-references-from (url)
  (collecting-set (:test #'equal)
    (mapcar #'gadgets:collect (grab-column (liql url 'reference2.fromurl 'tourl)))
    (mapcar #'gadgets:collect
            (grab-column (liql url 'opinion.url 'opinion 'reference.reference)))))

(defun get-references-to (url)
  (collecting-set (:test #'equal)
    (mapcar #'gadgets:collect (grab-column (liql url 'reference2.tourl 'fromurl)))
    (mapcar #'gadgets:collect
            (grab-column
              (liql url 'reference.reference 'reference 'opinion 'opinion.url)))))
