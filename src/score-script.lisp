...
(defun execute-score-script (scsc rooturl opinion-store)
  (let ((warflagger:*opinion-store* opinion-store)
        (scsc (sss:prep-scsc-for-execution (sss:scsc-safety-symbols scsc)))
        (sss:*warstats* (make-hash-table :test #'equal))
        (sss:*text-warstats* (make-hash-table :test #'equal))
        (sss:*title-warstats* (make-hash-table :test #'equal))

        (sss:*ballot-box* (warflagger:make-ballot-box))
        (sss:*text-ballot-box* (warflagger:make-ballot-box))
        (sss:*title-ballot-box* (warflagger:make-ballot-box))
        (sss:*warstat* (make-hash-table))
        (sss:*text-warstat* (make-hash-table))
        (sss:*title-warstat* (make-hash-table))
        (sss:*tree-address* nil))
    (mapc #'eval scsc)
    (unless (warflagger:ballot-box-empty-p sss:*ballot-box*)
      (warflagger:apply-ballot-box-to-warstats sss:*ballot-box* sss:*warstat*)
      (sss:collect-warstats rooturl sss:*warstat*))
    (unless (warflagger:ballot-box-empty-p sss:*text-ballot-box*)
      (warflagger:apply-ballot-box-to-warstats sss:*text-ballot-box* sss:*text-warstat*)
      (sss:collect-warstats rooturl sss:*text-warstat* :text))
    (unless (warflagger:ballot-box-empty-p sss:*title-ballot-box*)
      (warflagger:apply-ballot-box-to-warstats sss:*title-ballot-box* sss:*title-warstat*)
      (sss:collect-warstats rooturl sss:*title-warstat* :title))
    (values sss:*warstats* sss:*text-warstats* sss:*title-warstats*)))
...
(defun negative-spam (&key iid author modifiers)
  (sss:flag-core :spam nil iid author modifiers))
(defun negative-inflammatory (&key iid author modifiers)
  (sss:flag-core :inflammatory nil iid author modifiers))
...
(defun negative-language-warning (&key iid author modifiers)
  (sss:flag-core :language-warning nil iid author modifiers))
(defun negative-disturbing (&key iid author modifiers)
  (sss:flag-core :disturbing nil iid author modifiers))
(defun negative-logical-fallacy (&key iid author modifiers)
  (sss:flag-core :logical-fallacy nil iid author modifiers))
...
(defun negative-out-of-bounds (&key iid author modifiers)
  (sss:flag-core :out-of-bounds nil iid author modifiers))
(defun positive-funny (&key iid author modifiers)
  (sss:flag-core :funny nil iid author modifiers))
...
(defun positive-interesting (&key iid author modifiers)
  (sss:flag-core :interesting nil iid author modifiers))
...
(defun custodial-out-of-date (&key iid author modifiers)
  (sss:flag-core :out-of-date nil iid author modifiers))
(defun custodial-retraction (&key iid author modifiers)
  (if (equal author sss:*target-author*)
      (progn
        (sss:flag-core nil nil iid author modifiers)
        (setf (gethash :retraction sss:*warstat*) t))
...
(defun custodial-incorrect-flag (&key iid author modifiers)
  (sss:flag-core :incorrect-flag nil iid author modifiers))
(defun custodial-flag-abuse (&key iid author modifiers)
  (if (length1 (assoc-cdr :tree-address (warflagger:opinion-by-id iid)))
      ...
      (progn
        ...
        (sss:flag-core :flag-abuse nil iid author modifiers)
        (when (gethash :flag-abuse sss:*warstat*)
          (setf sss:*direction* :neutral)
          (setf sss:*other-flag* nil)))))
(defun custodial-offtopic (&key iid author modifiers)
  (sss:flag-core :offtopic nil iid author modifiers))
(defun custodial-arcane (&key iid author modifiers)
  (sss:flag-core :arcane nil iid author modifiers))
...
(defun target-text (&key iid author)
  ...
  (if (not (length1 sss:*tree-address*))
      (warn "Can't target the text of a non-rooturl target")
      (sss:apply-to :text iid)))
